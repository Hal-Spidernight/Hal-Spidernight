...
`npx hal-spidernight` で表示されるプロフィールカードの開発・公開手順。
...
```sh
npm run release:patch   # 0.2.0 → 0.2.1   (bug fix)
npm run release:minor   # 0.2.0 → 0.3.0   (機能追加・後方互換)
npm run release:major   # 0.2.0 → 1.0.0   (破壊的変更)
```
